<?php
$email = $phone = $updates = $coverage = $business = $business_contact = $business_type = $business_gross = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($business) || empty($business_contact) || empty($email) || empty($phone) || empty($coverage) || empty($business_type) ||  empty($business_gross)) { 
	...
	$txt = "Business Name: {$_POST['business']}<br> Business Contact: {$_POST['business_contact']} <br> Email: {$_POST['email']} <br> Phone: {$_POST['phone']} <br> <br> Coverage Amount: {$_POST['coverage']} Business Type: {$_POST['business_type']} <br> Gross Sales: {$_POST['business_gross']}";
