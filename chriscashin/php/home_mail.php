...
$firstName = $lastName = $address = $suite = $city = $state = $zip = $phone = $email = $updates = $msg = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($firstName) || empty($lastName) || empty($address) || empty($city) || empty($state) || empty($zip) || empty($email) || empty($phone)) { 
	...
	$txt = "First Name: {$_POST['firstName']}<br> Last Name: {$_POST['lastName']}<br> Address: {$_POST['address']} <br> Suite/Unit: {$_POST['suite']} <br> City: {$_POST['city']} <br> State: {$_POST['state']} <br> Zip Code: {$_POST['zip']} <br> Email: {$_POST['email']} <br> Email Updates: {$_POST['updates']} <br> Phone: {$_POST['phone']} <br> Message: {$_POST['msg']}";
